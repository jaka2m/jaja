...
>
    <script>
      (function() {
        // Ambil ID 'html' dari luar blok ini (asumsi ada <html id="html"> di luar)
        const htmlElement = document.documentElement; 
        const theme = localStorage.getItem('theme');
        
        // Terapkan 'dark' jika tema tersimpan adalah 'dark' atau jika belum ada tema (asumsi default dark)
        if (theme === 'dark' || !theme) {
          // Mengganti 'document.getElementById('html')' yang tidak ada di dalam skrip
          // dengan 'document.documentElement' (tag <html>)
...
<div class="mt-3">
            ...
            <div class="filter-container w-full max-w-5xl mb-6 p-6 bg-gray-800 rounded-xl shadow-xl grid grid-cols-2 md:grid-cols-4 gap-4 mx-auto" style="box-shadow: 0 4px 15px rgba(0,0,0,0.5), inset 0 0 10px rgba(0,0,0,0.2);">
            ...
            <div class="flex flex-col md:flex-row gap-4 w-full max-w-7xl justify-center mx-auto">
            ...
            <div class="mt-2"></div>
            <div class="flex flex-col md:flex-row gap-4 w-full max-w-7xl justify-center mx-auto">
    <div class="p-4 sm:p-6 bg-gray-800 rounded-xl shadow-lg">
        ...
        <div class="text-center mt-4">
            <button onclick="clearCreatedAccounts()" class="px-2 py-1 text-white rounded-lg disabled:opacity-50 text-base font-semibold btn-gradient hover:opacity-80 transition-opacity">
                Clear List
            </button>
        </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    async function displayCreatedAccounts(page = 1) {
        ...
        // Build table
        let tableHTML = '<div class="bg-gray-800/50 rounded-lg shadow-xl p-4">';
        tableHTML += '<div class="overflow-x-auto">';
        tableHTML += '<table class="min-w-full bg-gray-800/50 rounded-lg">';
        tableHTML += '<thead><tr>';
        
        tableHTML += '<th class="px-4 py-2 text-center text-sm font-bold text-white">No.</th>';
        tableHTML += '<th class="px-4 py-2 text-center text-sm font-bold text-white">Nama Remarks</th>';
        tableHTML += '<th class="px-4 py-2 text-center text-sm font-bold text-white">Protocol</th>';
        tableHTML += '</tr></thead><tbody>';

        paginatedAccounts.forEach((account, index) => {
            tableHTML += '<tr class="border-t border-gray-700">';
            tableHTML += '<td class="px-4 py-2 text-white font-bold text-center">' + (startIndex + index + 1) + '</td>';
            tableHTML += '<td class="px-4 py-2 text-white font-bold text-center">' + account.name + '</td>';
            tableHTML += '<td class="px-4 py-2 text-white font-bold text-center">' + account.protocol.toUpperCase() + '</td>';
            tableHTML += '</tr>';
        });
        ...
        if (totalPages > 1 && paginationContainer) {
            ...
            // Previous button
            if (page > 1) {
                paginationHTML += '<li><button onclick="navigateToAccountPage(' + (page - 1) + ')" class="px-2 py-1 text-white rounded-lg btn-gradient hover:opacity-80">Prev</button></li>';
            }

            // Page number buttons
            for (let i = 1; i <= totalPages; i++) {
                const activeClass = i === page ? 'bg-blue-700' : '';
                paginationHTML += '<li><button onclick="navigateToAccountPage(' + i + ')" class="px-2 py-1 text-white rounded-lg btn-gradient hover:opacity-80 ' + activeClass + '">' + i + '</button></li>';
            }

            // Next button
            if (page < totalPages) {
                paginationHTML += '<li><button onclick="navigateToAccountPage(' + (page + 1) + ')" class="px-2 py-1 text-white rounded-lg btn-gradient hover:opacity-80">Next</button></li>';
            }
        ...
        <div class="bg-gray-800/50 rounded-lg shadow-xl p-4"> 
            <div class="overflow-x-auto"> 
                <table class="min-w-full bg-gray-800/50 rounded-lg"> 
                    <thead>
                        <tr>
                            <th class="px-4 py-2 text-center text-sm font-bold text-white">No.</th> 
<th class="px-4 py-2 text-center text-sm font-bold text-white">IP</th> 
<th class="px-4 py-2 text-center text-sm font-bold text-white">Status</th> 
<th class="px-4 py-2 text-center text-sm font-bold text-white">Country</th> 
<th class="px-4 py-2 text-center text-sm font-bold text-white">ISP</th> 
<th class="px-4 py-2 text-center text-sm font-bold text-white">Action</th> 
</tr>
                    </thead>
                    <tbody>
                        ${tableRows}
